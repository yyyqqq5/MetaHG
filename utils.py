...
def get_taskembedding(data_dir):

    key_embedding_dict = dict()
    ...
    for line in f.readlines():
        ...
        if label not in key_embedding_dict.keys():
            key_embedding_dict[label] = [feature]
        else:
            key_embedding_dict[label].append(feature)
    key_ave_embedding_dict = dict()
    for key, value in key_embedding_dict.items():
        ave_array = np.array(pd.DataFrame(value).astype('float32')).mean(axis=0)
        key_ave_embedding_dict[key] = torch.from_numpy(ave_array)

    return key_ave_embedding_dict

# get_taskembedding(r"merge_embedding.txt")

def get_train_data(features, labels, select_label, k_spt, k_qry, batch_num):
    x_spt = []
    y_spt = []
    x_qry = []
    y_qry = []
    ...
    for t in range(batch_num):
        ...
        # print(class2_idx[t])
        # print(select_class[random_int][1])
...
def get_test_data(features, labels, select_label, k_spt, k_qry, batch_num):
    x_spt = []
    y_spt = []
    x_qry = []
    y_qry = []
...
def get_gcnlearndata(labels, fs_label):
    label_index, fewshot_index = [], []
    for i in range(len(labels)):
        if labels[i] != fs_label:
            label_index.append(i)
        else:
            fewshot_index.append(i)

    random.shuffle(label_index)
    random.shuffle(fewshot_index)

    train_ind = label_index[:int(len(label_index) * 0.8)]
    val_ind = label_index[int(len(label_index) * 0.8):int(len(label_index) * 0.9)]
    test_ind = label_index[int(len(label_index) * 0.9):] + fewshot_index[:20]

    train_idx = torch.tensor(train_ind).long()
    val_idx = torch.tensor(val_ind).long()
    test_idx = torch.tensor(test_ind).long()
...
def print_performance(epoch, F1, acc):
    ...
    # print("{} epoch: |F1: {:.4f} |accuracy: {:.4f}|recall: {:.4f} |precision: {:.4f}".format(epoch,F1, acc, r, p))
    print("{} epoch: |F1: {:.4f} |accuracy: {:.4f}".format(epoch, F1, acc))